...
input_tensor = jax.random.normal(key=jax.random.PRNGKey(11), shape=(4, 7))

# Initialize Conv1d layer
inp_channels = 7  # Input channels must match the last dimension of reshaped_input
...
conv_layer = eqx.nn.Conv1d(inp_channels, out_channels, kernel_size, padding=kernel_size // 2, key=key, use_bias=False)
